...
long long int seeds_range[MAX_RANGE];
int seed_count;
...
#define MAX_BUFF (1024)
    ...
    for(i=0;i<seed_count;i++) {
        ...
        outcount=0;
        minicount=0;
        for(lcount=seeds_used[i];lcount<(seeds_used[i]+seeds_range[i]);lcount++) {

        outcount++;
        minicount++;
        if(minicount>=1000000) {
            printf("%15lld  %15lld  %15lld\r",outcount,lcount,smallest_location);
            minicount=0;
        }            
        //printf("%lld ",lcount);
        tmp=rn_find(seed_to_soil,lcount);
        //printf("%lld ",tmp);
